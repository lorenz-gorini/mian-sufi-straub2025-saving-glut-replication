...
- `build_presentation.mjs` edits only mapped, inherited elements and replaces
  four inherited chart frames with generated Figure 8 outputs.
...
The rebuild requires `RUNTIME_NODE` and `RUNTIME_NODE_MODULES` from the
presentation runtime dependency loader. It does not install dependencies into
the repository.
