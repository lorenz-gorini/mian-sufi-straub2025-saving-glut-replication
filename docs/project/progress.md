...
| T-003 | Reconstruct cleaning and merges | In progress | 2 | data-engineering |  | Reimplement the minimum required preparation with explicit transformations and merge contracts. | Figure 1 now reconstructs the four intermediary-bond cells and the household equity share from the kit's wide Q4 panel; final files are retained only for validation and unavailable downstream denominators. After review, reconstruct the earliest feasible Figure 5 pipeline. | [Figure 1 data](../data/figure1-data.md) |
| T-004 | Reproduce selected computations and exhibits | In progress | 1 | replication | 120m | Generate Figures 1, 5, and 8 as closely as the supplied inputs permit and compare each with declared benchmark evidence. | Figure 1 now has two explicitly separate builds: the primary authors-kit reconstruction through 2016 (bond cells within \$0.05 million; level correlation 0.995) and a secondary public-data robustness result through 2019. Review their documented boundary before starting Figure 5. | [Figure 1 replication](tasks/figure1-replication.md); [Figure 1 data](../data/figure1-data.md); [major results](../../Results_Proposal/major_results.md) |
...
1. Review the documented Figure 1 reconstruction: the numerator is rebuilt
   from the authors' Q4 panel and raw CRSP data; preserved final files enter
   only as validation evidence and for unavailable denominator/NI objects.
