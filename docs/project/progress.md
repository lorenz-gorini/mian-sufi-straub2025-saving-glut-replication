...
| T-012 | Assemble the written submission | Complete | 1 | writing |  | Combine the referee report, replication summary, and extension proposal without duplicating document environments or preambles. | Each component now has a standalone `main.tex` and a body-only source; `Submission_Report/main.tex` imports the three bodies in order. All four entry points compile, the combined artifact has the intended 2+3+2 page structure, and all seven pages pass visual inspection. | [Submission-report task](tasks/submission-report.md); [combined entry point](../../Submission_Report/main.tex); [combined PDF](../../Submission_Report/submission_report.pdf) |
