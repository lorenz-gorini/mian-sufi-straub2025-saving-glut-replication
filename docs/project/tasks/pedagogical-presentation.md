...
| 3 | Figure 6 | Explain how the active-saving numerator and disposable-income denominator produce saving rates across the wealth distribution. |
| 4 | Figure 8 | Explain how unveiled debt assets and household liabilities determine net debt across wealth groups. |
| 5 | Figure 9 | Optional extension from debt stocks to financing flows if the core figures are complete. |
...
5. Figure 1, Figure 5, Figure 6, and Figure 8 comparison panels, each showing the 2021
...
6. Figure 6 methodology and reconstruction.
7. Figure 8 methodology and reconstruction.
8. Figure 9 extension, if reached.
9. Comparison summary, limitations, and lessons learned.
10. Appendix: schemas, merge diagnostics, accounting residuals, and robustness.

## Definition of done

- [x] The package-validation, data-map, and theoretical-methodology source
      modules exist and have been visually verified.
- [ ] The refreshed synthesis deck has one 16:9 entry point that builds
      reproducibly.
- [x] Figures 1, 5, 6, and 8 each have a complete intuitive, mathematical, data,
      implementation, and result explanation.
- [x] Every empirical value, figure, and table is generated by code.
- [x] Notation, units, signs, group colors, and base periods are consistent.
- [x] Benchmark and independent results are visually and quantitatively
      distinguished.
- [x] Figure 9 is included only if the core scope is complete.
- [x] Appendix diagnostics support rather than interrupt the main explanation.
- [ ] The refreshed rendered deck has been visually inspected for legibility.
- [ ] Benchmark legends and labels remain readable without materially shrinking
      the data panels.
- [x] NotebookLM-inspired visuals are independently recreated and all factual
      content is checked against the 2025 paper or project evidence.

The final deck intentionally omits Figure 9 and a standalone appendix. The
verified topic modules retain the detailed diagnostics, while the submitted
sequence stays focused on the three approved core figures. The unchecked
items are the export and visual-QA boundary for the pending 18-slide refresh;
the currently stored 17-slide PowerPoint passed those checks before this task
was reopened.
...
The seven-slide `Presentation/4_figure_comparison/presentation.pptx` is now the
maintained empirical-comparison module. It contains only the direct July 2025
paper-versus-our-implementation comparisons for Figures 1, 5, 6, and 8.
Figures 1, 5, and 6 use the 2025 definitions with 2021-kit inputs; Figure 8
uses the saved 2021 direct/pre-round fields and the full inverse. The old
seven-round benchmark and public-FWTW robustness remain in the separate
focused Figure 8 deck. The Figure 6 raster, wealth-level, and time-evolution
diagnostics no longer interrupt the direct comparison; the wealth and heat-map
views now support the debt-composition extension. All seven comparison slides
were rendered and inspected individually on 2026-08-16, contain source blocks
in speaker notes, and pass template-plan, template-fidelity, overflow,
unresolved-placeholder, and archive checks.

## Session record: 2026-08-16 comparison-deck scope correction

- Goal: make the empirical module a direct paper-versus-implementation
  comparison and incorporate the completed Figure 6 and Figure 8 results.
- Changed: the Figure Comparison deck, its template-following source and
  provenance files, the Debt-Composition Extension deck and source, the
  presentation collection contract, this task record, the extension task
  record, and the dashboard.
- Result: the comparison module is seven slides: contract, Figures 1/5/6/8,
  and synthesis. Figure 8 now uses the 2021-direct/full-inverse output rather
  than the old seven-round proxy. Figure 6 wealth-level and trailing-five-year
  diagnostics moved to the seven-slide extension module.
- Validation: every slide in both decks was inspected individually; both pass
  template-plan, template-fidelity, overflow, placeholder, speaker-note source,
  and archive-integrity checks.
- Next action: review the focused module, then refresh the final submission
  synthesis without reintroducing diagnostic-only slides.
...
| 2026-08-16 | Keep deck 4 comparison-only and move Figure 6 mechanism diagnostics to deck 7. | Paper-versus-implementation judgments should not be interrupted by new wealth/time views; those diagnostics instead sharpen the mortgage-extension motivation. |
| 2026-08-16 | Use the 2021-direct/full-inverse route on the Figure 8 comparison slide. | It is the earliest feasible same-vintage reconstruction of the 2025 operator and supersedes the old seven-round proxy as the primary implemented result. |
...
## Session record: 2026-08-14 submission-deck source refresh

- Refactored the synthesis builder to select 18 slides: seven theory slides,
  five data/version slides, and six result/synthesis slides.
- Replaced the old Figure 8 proxy sequence with the focused estimand, primary
  same-vintage comparison, and same-input operator diagnostic from the
  eight-slide method module.
- Added the project-built Leontief network cover visual. The opening pacing is
  inspired by the initial NotebookLM synthesis pages, but the secondary
  synthesis supplies no equations or evidence.
- Rewrote the closing slide around three declared reconstruction boundaries:
  Figures 1 and 5 from old authors' inputs and Figure 8 from saved direct
  fields with a coarsened full-inverse network.
- Validation: `node --check` passes. The required Artifact Tool runtime
  dependency loader was unavailable, so the refreshed deck could not be
  exported, rendered, or visually inspected. The previously verified
  17-slide `presentation.pptx` therefore remains unchanged until the 18-slide
  source can be rebuilt and fully checked.
...
## Session record: 2026-08-13 focused Figure 8 method comparison

- Implemented the previously identified public-FWTW full-Leontief rerun in a
  separate Python pipeline without changing the retained proxy.
- Added `Presentation/6_figure8_method_comparison/presentation.pptx`, an
  eight-slide module comparing the shared estimand, inputs, operator,
  accounting checks, paper fit, and evidentiary boundary.
- Recommendation: use the public-FWTW full-Leontief reconstruction as the
  preferred Figure 8 submission result and retain the proxy as lineage; do not
  claim a pure code effect or an exact authors-vintage reproduction.

## Session record: 2026-08-13 final replication submission

- Goal: assemble one concise editable deck from the verified theory, data-map,
  and empirical-comparison modules without overwriting those source decks.
- Output: `Presentation/5_replication_submission/presentation.pptx`, a 17-slide
  sequence covering the ownership veil, Equation (4), the Leontief solve, the
  worked network, data roles and vintages, and the Figure 1/5/8 comparisons.
- Evidence boundary: Figures 1 and 5 are old-input reconstructions. Figure 8
  remains the explicitly labelled seven-round proxy; public FWTW is described
  only as a future mixed-vintage method test.
- Visual inspiration: the first NotebookLM pages influenced the veil-to-network
  pacing only. All equations, diagrams, claims, and citations come from the
  target paper or verified project evidence.
- Reproducibility: the build script imports native editable slides, remaps
  colliding media identifiers, preserves `[Sources]` speaker notes, and writes
  the merged PowerPoint without modifying any source deck.
- Validation: all 17 slides were rendered and inspected at full resolution;
  the automated overflow test passed, every note contains a source block, the
  deck has no unresolved placeholders, and the PPTX archive validates.
